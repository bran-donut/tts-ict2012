...
export default function Schedule() {
    ...
    return (
        <Layout>
            ...
            <SubHeader heading={(index == 0 && 'Edit' || index == 1 && 'Add to' || index == 2 && 'Remove from') + " Sample Schedule"} description="Displays equipment to be added to the schedule" breadCrumbItems={['Home', 'Schedule', tabs[index]]}>
                ...
                {index == 1 ?
                    <ItemWrapper>
                        {equipments.map((item, i) => {
                            ...
                            if (display && !item.sampleDate) return (
                                <ItemCard
                                    key={i}
                                    index={i}
                                    data={item}
            ...
            <div className="flex flex-col items-center justify-end w-full gap-10 px-5 py-5 bg-white md:flex-row">
                ...
                {/* add */}
                {index == 1 &&
                    <button type="submit" onClick={() => addItem.length ? setShowModal(true) : null} className={`px-5 py-2 text-white transition-colors duration-150 border-2 rounded-sm ${addItem.length ? 'bg-tts-red hover:bg-tts-red/80 border-tts-red' : 'bg-gray-400 border-gray-400 cursor-default'}`}>
                        Add
                    </button>
                }
                ...
                {/* remove */}
                {index == 2 &&
                    <button type="submit" onClick={() => removeItem.length ? setShowModal(true) : null} className={`px-5 py-2 text-white transition-colors duration-150 border-2 rounded-sm ${removeItem.length ? 'bg-tts-red hover:bg-tts-red/80 border-tts-red' : 'bg-gray-400 border-gray-400 cursor-default'}`}>
                        Remove
                    </button>
                }
                ...
                <>
                    {index == 0 &&
                        <Modal heading="Edit Next Sample Date" description="" leftText="Cancel" rightText="Done" onClickClose={handleCloseEditModal}>
                            <table cellPadding="9" className="table-fixed mx-8 mb-10">
                                <tbody>
                    ...
                    {index == 1 && <Modal heading="Add to Sample Schedule" description="Are you sure you want to add the equipments to sample schedule?" leftText="Cancel" rightText="Add" onClickClose={handleCloseModal} />}
                    {index == 2 && <Modal heading="Remove from Sample Schedule" description="Are you sure you want to remove the equipments from Sample Schedule?" leftText="Cancel" rightText="Remove" onClickClose={handleCloseModal} />}
                </>
            }

            {/* confirm edit changes modal */}
            {showConfirmEditModal && <Modal heading="Edit Sample Schedule" description="Do you want to save the changes to Sample Schedule?" leftText="Cancel" rightText="Done" onClickClose={handleCloseEditConfirmModal} />}
        </Layout>
    )
}
